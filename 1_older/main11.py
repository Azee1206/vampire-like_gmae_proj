...
class Player(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.current_exp >= self.exp_for_next_level:
            ...
            # временное решение
            self.current_hp += self.specifications['health'] * 0.15
            # меню апгрейда
...
class ItemShop:
    ...
    def buy_item(self, index):
        for char in character:
            ...
            # учесть случай выбора уже купленного предмета
            if char.hoard_exp >= item.cost:
...
def upgrade_menu(screen):
    ...
    # исправить максимальный индекс
    shop = ItemShop(sec_counter, 4)
    ...
    for i in range(len(pictures)):
        buttons.append(
            Button((left_start + step * i, top_start), f"{i}. Купить"))
    i += 1
                ...
                for button in buttons:
                    if button.release(event.pos):
                        if button.get_text() == "Закрыть":
                            ...
                            shop.buy_item(int(button.get_text()[0]))
        screen.fill(STRANGE_GREEN)

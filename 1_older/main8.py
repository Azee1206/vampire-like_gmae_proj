...
class Weapon(pygame.sprite.Sprite):
    ...
    def shoot(self, direction):
        ...
        # разброс
        if self.diff != 0:
            direction += randrange(-self.diff, self.diff)
        # собственно, снаряд
...
def text_object(string, pos_x, pos_y, size=40, color=BLACK):
...
def menu(screen):
    ...
    for string in text:
        ...
        start += 50
    # текст
    string_rendered, text_pos = text_object("Пауза", width // 2 - 75,
                                            height // 3)
    while running:
...
def start_screen(screen):
    ...
    running = True
    # текст
    string_rendered, text_pos = text_object(
